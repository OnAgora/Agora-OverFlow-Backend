...
pub contract YoungApeDiaries: NonFungibleToken {
  ...
  pub event Purchase(id: UInt64, recipient: Address, metadataId: UInt64, name: String, description: String, image: MetadataViews.IPFSFile, price: UFix64)
	...
	pub struct NFTMetadata {
		...
		init(_name: String, _description: String, _image: MetadataViews.IPFSFile, _thumbnail: MetadataViews.IPFSFile?, _price: UFix64?, _extra: {String: AnyStruct}, _supply: UInt64) {
			self.metadataId = YoungApeDiaries.nextMetadataId
			self.name = _name
	...
	pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
		...
		pub fun getMetadata(): NFTMetadata {
			return YoungApeDiaries.getNFTMetadata(self.metadataId)!
		}
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: YoungApeDiaries.CollectionStoragePath,
						publicPath: YoungApeDiaries.CollectionPublicPath,
						providerPath: YoungApeDiaries.CollectionPrivatePath,
						...
						createEmptyCollectionFunction: (fun (): @NonFungibleToken.Collection {
								return <- YoungApeDiaries.createEmptyCollection()
						})
					)
        ...
        case Type<MetadataViews.NFTCollectionDisplay>():
          let squareMedia = MetadataViews.Media(
            file: YoungApeDiaries.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
            mediaType: "image"
          )
					...
					if let bannerImage = YoungApeDiaries.getOptionalCollectionAttribute(key: "bannerImage") as! MetadataViews.IPFSFile? {
        ...
        return MetadataViews.NFTCollectionDisplay(
          name: YoungApeDiaries.getCollectionAttribute(key: "name") as! String,
          description: YoungApeDiaries.getCollectionAttribute(key: "description") as! String,
          externalURL: MetadataViews.ExternalURL("https://nporium.com"),
          squareImage: squareMedia,
          bannerImage: bannerMedia ?? squareMedia,
          socials: YoungApeDiaries.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
        )
		...
		init(_metadataId: UInt64, _serial: UInt64, _recipient: Address) {
			pre {
				YoungApeDiaries.metadatas[_metadataId] != nil:
					"This NFT does not exist yet."
				_serial < YoungApeDiaries.getNFTMetadata(_metadataId)!.supply:
					"This serial does not exist for this metadataId."
				!YoungApeDiaries.getNFTMetadata(_metadataId)!.purchasers.containsKey(_serial):
			...
			if let buyersRef = &YoungApeDiaries.primaryBuyers[_recipient] as &{UInt64: [UInt64]}? {
				...
				YoungApeDiaries.primaryBuyers[_recipient] = {_metadataId: [_serial]}
			}

			// Update who bought this serial inside NFTMetadata so it cannot be purchased again.
			let metadataRef = (&YoungApeDiaries.metadatas[_metadataId] as &NFTMetadata?)!
			metadataRef.purchased(serial: _serial, buyer: _recipient)

			YoungApeDiaries.totalSupply = YoungApeDiaries.totalSupply + 1
	...
	pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection {
		...
		pub fun claim() {
			if let storage = &YoungApeDiaries.nftStorage[self.owner!.address] as &{UInt64: NFT}? {
	...
	pub fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FlowToken.Vault, serial: UInt64): UInt64 {
		...
		/*if let royalty = YoungApeDiaries.getOptionalCollectionAttribute(key: "royalty") as! MetadataViews.Royalty? {
	...
	pub fun createNFTMetadata(
		...
		supply: UInt64) {
			YoungApeDiaries.metadatas[YoungApeDiaries.nextMetadataId] = NFTMetadata(
				_name: name,
			...
			YoungApeDiaries.nextMetadataId = YoungApeDiaries.nextMetadataId + 1
		}

		// mintNFT mints a new NFT and deposits
		// it in the recipients collection
		// This function doesn't require payment
		...
		pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address) {
			...
			if let recipientCollection = getAccount(recipient).getCapability(YoungApeDiaries.CollectionPublicPath).borrow<&YoungApeDiaries.Collection{NonFungibleToken.CollectionPublic}>() {
				recipientCollection.deposit(token: <- nft)
			} else {
				if let storage = &YoungApeDiaries.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
					YoungApeDiaries.nftStorage[recipient] <-! {nft.id: <- nft}
				}
			}
		}
		...
		pub fun changeField(key: String, value: AnyStruct) {
			YoungApeDiaries.collectionInfo[key] = value
		}
	}
	...
	pub fun getPriceOfNFT(_ metadataId: UInt64): UFix64? {
		if let metadata: YoungApeDiaries.NFTMetadata = self.getNFTMetadata(metadataId) {
	...
	pub fun getClaimableNFTs(user: Address): {UInt64: NFTMetadata} {
		...
		if let storage = &YoungApeDiaries.nftStorage[user] as &{UInt64: NFT}? {
			for id in storage.keys {
    ...
    self.collectionInfo["minting"] = _minting
		self.collectionInfo["royalty"] = _royalty
    ...
    self.collectionInfo["profit"] = 0.0
		...
		self.CollectionStoragePath = /storage/YoungApeDiariesCollection_0x61449dd762defed0
		self.CollectionPublicPath = /public/YoungApeDiariesCollection_0x61449dd762defed0
		self.CollectionPrivatePath = /private/YoungApeDiariesCollection_0x61449dd762defed0
		self.AdministratorStoragePath = /storage/YoungApeDiariesAdministrator_0x61449dd762defed0
